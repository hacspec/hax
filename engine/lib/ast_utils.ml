      ...
      | TY ->
          [
            ...
            (fun () ->
              ...
              ([%yojson_of: ty] (TApp { ident = g_ident; args = [GType (typ) (* must have 1+ items *)] }), indexes));
            (fun () ->
              let typ, indexes = generate_helper TY indexes in
              let typ = [%of_yojson: ty] typ in
            ...
            (fun () ->
              ...
              ( [%yojson_of: ty] (TDyn { witness = wit; goals = [] }),
                indexes ));
          ]
      | EXPR ->
          ...
          List.map
            ...
            [
              ...
              (fun () ->
                ...
                let wit = [%of_yojson: F.mutable_variable] (`String "mutable_variable") in
              ...
              (fun () ->
                ...
                let wit = [%of_yojson: F.question_mark] (`String "Question_mark") in
  ...
  let rec generate_depth depth (pre : int list) (t : ast_type) : int list list =
    List.map
      ...
      | TY ->
          ...
          (* TSlice *)
          @ (try let _ = [%of_yojson: F.slice] (`String "Slice") in generate_depth_list_helper (depth - 1) [ 7 ] [ TY ] with _ -> [])
          @ (try let _ = [%of_yojson: F.raw_pointer] (`String "Raw_pointer") in [ (* TRawPointer *) [ 8 ] ] with _ -> [])
          (* TRef *)
          @ (try let _ = [%of_yojson: F.reference] (`String "Reference") in generate_depth_list_helper (depth - 1) [ 9 ] [ TY ] with _ -> []) (* TODO: mutable? *)
          (* TParam *)
          ...
          @ (try let _ = [%of_yojson: F.dyn] (`String "Dyn") in [ (* TDyn *) [ 14 ] ] with _ -> [])
      | PAT ->
          List.map
            ~f:(fun x ->
              x @ [ 0 ]
            ...
            @ [ (* PArray *) [ 4 ] ]
            (* PDeref *)
            @ (try let _ = [%of_yojson: F.reference] (`String "Reference") in generate_depth_list_helper (depth - 1) [ 5 ] [ PAT ] with _ -> [])
            (* PConstant *)
            @ generate_depth_list_helper depth [ 6 ] [ LITERAL ]
            @ (* PBinding *)
            (try let _ = [%of_yojson: F.mutable_variable] (`String "Mutable_variable") in generate_depth_list_helper (depth - 1) [ 7 ] [ LOCAL_IDENT; TY ] with _ -> []))
      | EXPR ->
          List.map
            ~f:(fun x ->
              x @ [ 0 ]
            ...
            (* Block *)
            @ (try let _ = [%of_yojson: F.block] (`String "Block") in generate_depth_list_helper (depth - 1) [ 7 ] [ EXPR ] with _ -> [])
            (* LocalVar *)
            ...
            (* MacroInvokation *)
            @ (try let _ = [%of_yojson: F.macro] (`String "Macro") in generate_depth_list_helper (depth - 1) [ 11 ] [ GLOBAL_IDENT ] with _ -> [])
            (* Assign *)
            @ (try let _ = [%of_yojson: F.mutable_variable] (`String "Mutable_variable") in generate_depth_list_helper (depth - 1) [ 12 ]
                [ LOCAL_IDENT; EXPR; TY ] with _ -> [])
            (* Loop *)
            @ (try let _ = [%of_yojson: F.loop] (`String "Loop") in generate_depth_list_helper (depth - 1) [ 13 ] [ EXPR ] with _ -> [])
            (* Break *)
            @ (try let _ = [%of_yojson: F.loop] (`String "Loop") in let _ = [%of_yojson: F.break] (`String "Break") in generate_depth_list_helper (depth - 1) [ 14 ] [ EXPR ] with _ -> [])
            (* Return *)
            @ (try let _ = [%of_yojson: F.early_exit] (`String "Early_exit") in generate_depth_list_helper (depth - 1) [ 15 ] [ EXPR ] with _ -> [])
            (* QuestionMark *)
            @ (try let _ = [%of_yojson: F.question_mark] (`String "Question_mark") in generate_depth_list_helper (depth - 1) [ 16 ] [ EXPR; TY ] with _ -> [])
            @ (try let _ = [%of_yojson: F.loop] (`String "Loop") in let _ = [%of_yojson: F.continue] (`String "Continue") in [ (* Continue *) [ 17 ] ] with _ -> [])
            (* Borrow *)
            @ (try let _ = [%of_yojson: F.reference] (`String "Reference") in generate_depth_list_helper (depth - 1) [ 18 ] [ EXPR ] with _ -> [])
            (* AddressOf *)
            @ (try let _ = [%of_yojson: F.raw_pointer] (`String "Raw_pointer") in generate_depth_list_helper (depth - 1) [ 19 ] [ EXPR ] with _ -> [])
            @ (* Closure *)
      ...
      | ITEM ->
          List.concat_map
            ...
            @ generate_depth_list_helper (depth - 1) [ 3 ]
                [ CONCRETE_IDENT; GENERICS ]
            (* IMacroInvokation *)
            @ (try let _ = [%of_yojson: F.macro] (`String "Macro") in generate_depth_list_helper depth [ 4 ] [ CONCRETE_IDENT ] with _ -> [])
            (* Trait *)
