...
module%inlined_contents Make (F : Features.T) = struct
  ...
  let rec analyse (func_dep : pre_data) (items : A.item list) : analysis_data =
    let mut_var_list, _ = (* (U.TypedLocalIdent.t * id_order) list * id_order *)
      List.fold_left
        ~f:(fun (y, count) x ->
            match x.v with
        ...
        ~init:([], 0) items
    in
    let mut_map : (local_ident list * (U.TypedLocalIdent.t * id_order) list) Map.M(Concrete_ident).t
        =
      List.fold_left
        ~f:(fun y (x_name, x_items) -> Map.set y ~key:(x_name) ~data:(
