...
*)
let to_string ~(namespace : View.path) :
  ...
  in
  let adt def_id =
    let view = View.of_def_id (DefId.make Regular def_id) in
    let* () = [%equal: View.path] view.path namespace |> some_if_true in
    let* last = expect_singleton view.name_path in
    let* name =
      match last with
