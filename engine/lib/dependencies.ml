...
module Make (F : Features.T) = struct
  ...
  module type WITH_ITEMS = sig
    val uid_associated_items : item list -> Ast.attrs -> item list
    val bundle_cyclic_modules : item list -> item list
    val sort : item list -> item list
    val recursive_bundles : item list -> item list list * item list

    val filter_by_inclusion_clauses :
      Types.inclusion_clause list -> item list -> item list
  end

  module WithItems (CTX : sig
    val items : item list
  end) =
  struct
    let uid_associated_items : attrs -> item list =
    ...
    module ItemGraph = struct
      ...
      let vertices_of_items (items : item list) : G.E.t list =
        List.concat_map
          ~f:(fun i ->
            let assoc =
              uid_associated_items i.attrs |> List.map ~f:(fun i -> i.ident)
            in
            vertices_of_item i @ assoc
            |> List.map ~f:(Fn.const i.ident &&& Fn.id))
          items
      ...
      let transitive_dependencies_of (g : G.t)
          (selection : Concrete_ident.t list) : Concrete_ident.t Hash_set.t =
  ...
  let uid_associated_items items' =
    let open WithItems (struct
      let items = items'
    end) in
    uid_associated_items

  let bundle_cyclic_modules items' =
    let open WithItems (struct
      let items = items'
    end) in
    bundle_cyclic_modules items'
  ...
  let recursive_bundles items' =
    let open WithItems (struct
      let items = items'
    end) in
    recursive_bundles items'

  let filter_by_inclusion_clauses clauses items' =
    let open WithItems (struct
      let items = items'
    end) in
    filter_by_inclusion_clauses clauses items'
end
