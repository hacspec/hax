        ...
        | `Concrete Non_empty_list.{ crate = "hacspec_lib_tc"; path = [ name ] }
          -> (
            ...
            | "unsigned_public_integer" ->
                ...
                F.decls_of_string @@ "unfold type "
                ^ str_of_type_ident (hacspec_lib_item @@ o.integer_name)
                ^ "  = lseq uint8 ("
              ...
              | TIType bounds ->
                  ...
                  :: List.map
                       ~f:(fun { trait; args; bindings = _ } ->
                         let base = F.term @@ F.AST.Name (ptype_ident trait) in
                         let args = List.map ~f:pgeneric_value args in
                         ( F.id
                             (name ^ "_implements_" ^ last_of_global_ident trait),
                           None,
                           [],
                           F.mk_e_app base args ))
                       bounds
