...
module ProverifBackend = struct
  ...
  module RejectNotPV (FA : Features.T) = struct
    ...
    include
      Feature_gate.Make (FA) (FB)
        (struct
          ...
          let for_loop = reject
          let metadata = Desugar_utils.Metadata.make "RejectNotPV"
        end)
  end
  ...
  (* Helpers for constructing an PV surface AST *)
  module C = struct
    ...
    let rec pitem (e : item) : C.AST.decl list =
      ...
      | Type { name; generics; variants; record = true } ->
         [ C.AST.Inductive (pglobal_ident_last name ^ "_t",
                            List.fold_left ~init:[] ~f:(fun a b -> a @ [match b with | GPType {ident; default} -> ident.name | _ -> failwith "Proverif: TODO: generic_params"]) generics.params,
  ...
  let hardcoded_proverif_headers =
    "(* File automatically generated by Hacspec *)\n\
     From Hacspec Require Import Hacspec_Lib MachineIntegers.\n\
     From Proverif Require Import ZArith.\n\
...
let register = Backend.Registration.register (module ProverifBackend)
