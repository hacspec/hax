...
struct
        ...
        SSP.AST.ArrayTy
          ( pty span typ,
            ...
            ^ SSP.term_to_string_with_paren
                (pexpr (Map.empty (module LocalIdent)) false length)
                0
            ^ ")" ^ ")" )
  ...
  and pexpr (env : LocalIdentOrLisIis.W.t list Map.M(LocalIdent).t)
  ...
  and vars_from_pat : pat -> LocalIdent.t list =
    U.Reducers.variables_of_pat >> Set.to_list

  and env_from_param (params : pat list) :
      LocalIdentOrLisIis.W.t list Map.M(LocalIdent).t =
    Map.of_alist_exn
      (module LocalIdent)
      (List.concat_mapi
         ~f:(fun i pat ->
  ...
  and extend_env (env : LocalIdentOrLisIis.W.t list Map.M(LocalIdent).t)
      (env_ext : LocalIdentOrLisIis.W.t list Map.M(LocalIdent).t) :
      LocalIdentOrLisIis.W.t list Map.M(LocalIdent).t =
  ...
  and extend_env_with_params
      ...
      (params : pat list) : LocalIdentOrLisIis.W.t list Map.M(LocalIdent).t =
    extend_env env (env_from_param params)

  and analyse_env_of_expr
      (env : LocalIdentOrLisIis.W.t list Map.M(LocalIdent).t) (e : expr)
      extra_set =
  ...
  and both_type_expr (env : LocalIdentOrLisIis.W.t list Map.M(LocalIdent).t)
                         ...
                         | IIConst { body } ->
                             [
                               SSP.AST.LetDef
                                 ( pconcrete_ident x.ii_ident,
                                   [],
                                   (pexpr (Map.empty (module LocalIdent)) false)
                                     body,
                                   SSPExtraDefinitions.wrap_type_in_both
