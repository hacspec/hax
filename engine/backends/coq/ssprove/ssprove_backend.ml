...
(* module SsproveNamePolicy = Concrete_ident.DefaultNamePolicy *)
module SsproveNamePolicy = struct
...
open Ast
module U = Ast_utils.MakeWithNamePolicy (InputLanguage) (SsproveNamePolicy)
open AST

let hardcoded_ssprove_headers =
...
let dummy_lib =
  "(* TODO: Replace this dummy lib with core lib *)\n\
   Class t_Sized (T : choice_type) := { }.\n\
   Definition t_u8 := int8.\n\
   Definition t_u16 := int16.\n\
   Definition t_u32 := int32.\n\
   Definition t_u64 := int64.\n\
   Definition t_u128 := int128.\n\
   Definition t_usize := int64.\n\
   Definition t_i8 := int8.\n\
   Definition t_i16 := int16.\n\
   Definition t_i32 := int32.\n\
   Definition t_i64 := int64.\n\
   Definition t_i128 := int128.\n\
   Definition t_isize := int64.\n\
   Definition t_Array T (x : both t_usize) := chList T.\n\
   Axiom chString : choice_type.\n\
   Definition t_String := chString.\n\
   Axiom chFloat : choice_type.\n\
   Definition float := chFloat.\n\
   Axiom float_to_float : PrimFloat.float -> both chFloat.\n\
   Notation \"x '%float'\" := (float_to_float x) (at level 100).\n\
   Axiom ToString_f_to_string : string -> both chString.\n\
   Instance Sized_any : forall {t_A}, t_Sized t_A := {}.\n\
   Class t_Clone (T : choice_type) := { Clone_f_clone : T -> T }.\n\
   Instance Clone_any : forall {t_A}, t_Clone t_A := {Clone_f_clone := fun x \
   => x}.\n\
   Definition t_Slice (T : choice_type) := chList T.\n\
   Definition unsize {T : choice_type} : list T -> t_Slice T := id.\n\
   ...
   Definition t_Vec T (_ : globality) : choice_type := chList T.\n\
   ...
   Class t_From (A B : choice_type) := { From_f_from : B -> A }.\n\
   Definition impl__to_vec {T} (x : t_Slice T) : t_Vec T t_Global := x.\n\
   Class t_Into (A B : choice_type) := { Into_f_into : A -> B }.\n\
   Instance t_Into_from_t_From {A B : choice_type} `{H : t_From B A} : t_Into A B := \
...
struct
  ...
  module CoqNotation = struct
    ...
    let ssprove_definition name generics params typ body = definition name generics params (parens(!^"both " ^^ parens(typ))) body
    let ssprove_inductive name generics params typ body =
      definition name generics params typ body
  ...
  class printer =
    object (self)
      ...
      method expr'_Construct_tuple ~super:_ ~components =
        if List.length components == 0 then !^"ret_both" ^^ space ^^ !^"tt"
      ...
      method expr'_If ~super:_ ~cond ~then_ ~else_ =
        ...
        ^^ break 1 ^^ string "else"
        ^^ nest 2
             (break 1 ^^ match else_ with Some x -> x#p | None -> !^"ret_both" ^^ space ^^ !^"tt")
      ...
      method expr'_Match ~super:_ ~scrutinee ~arms =
        string "matchb" ^^ space ^^ scrutinee#p ^^ space ^^ string "with"
        ^^ break 1
        ^^ concat_map_with
             ~pre:(string "|" ^^ space)
             ~post:(break 1)
      ...
      method generic_param_kind_GPConst ~typ = parens(!^"both" ^^ space ^^ parens(typ#p))

      method generic_param_kind_GPLifetime ~witness =
        match witness with _ -> .

      method generic_param_kind_GPType = string "choice_type"
      ...
      method item'_Fn ~super ~name ~generics ~body ~params ~safety:_ =
        ...
        if is_lemma then
          ...
          CoqNotation.ssprove_definition name#p generics#p
            (params
            @ Option.value ~default:[]
                (Option.map ~f:(fun x -> [ string "`" ^^ braces x ]) requires))
            typ#p body#p (* ^^ TODO: ensures? *)
      ...
      method item'_Type_enum ~super ~name ~generics ~variants =
        ...
        ^^ CoqNotation.ssprove_inductive name#p
             (concat_map_with ~pre:space
                (fun x -> parens (self#entrypoint_generic_param x))
                generics#v.params
             ^^ concat_map_with ~pre:space
                  ...
                  generics#v.constraints)
             [] (string "choice_type")
             (separate_map (space ^^ string "∐" ^^ space) (fun x ->
                  parens(if List.is_empty x#v.arguments
                  then
                    string "chUnit"
                  else
                  separate_map (space ^^ !^"×" ^^ space)
                   (fun (ident, typ, attrs) ->
                     ( self#_do_not_override_lazy_of_ty
                         AstPos_variant__arguments typ )#p)
                   x#v.arguments)) variants)
        ...
        ^^ concat
          (List.mapi ~f:(fun i v ->
               (break 1) ^^
             CoqNotation.definition
               (self#_do_not_override_lazy_of_concrete_ident AstPos_variant__arguments v#v.name)#p
               (concat_map_with ~pre:space
                  (fun x -> parens (self#entrypoint_generic_param x))
                  generics#v.params
                ^^ concat_map_with ~pre:space
                  (fun x -> self#entrypoint_generic_constraint x)
                  generics#v.constraints)
               (if List.is_empty v#v.arguments then [] else (List.mapi ~f:(fun i (ident, typ, attrs) ->
                                                                         ( self#_do_not_override_lazy_of_ty
                                                                             AstPos_variant__arguments typ )#p)
                                                                      v#v.arguments)[parens(string "x" ^^ space ^^ colon ^^ space ^^ !^"both" ^^ space ^^ parens(separate_map (space ^^ !^"×" ^^ space)
                                                                      ))])
               (!^"both" ^^ space ^^
                parens(name#p ^^ concat_map_with ~pre:space
                          (fun (x : generic_param) ->
                            match x with
                            | { ident; _ } ->
                                (self#_do_not_override_lazy_of_local_ident
                                  ...
                                  #p)
                          generics#v.params))
               (if List.is_empty v#v.arguments
                then
                  (!^"ret_both" ^^ space ^^
                 parens(List.fold_left
                  ~init:(if List.is_empty v#v.arguments then !^"tt" else !^"x")
                  ~f:(fun y x -> parens(x ^^ space ^^ y))
                  ((if Stdlib.(i != 0) then [ !^"inr" ] else []) @ List.init (List.length variants - 1 - i) ~f:(fun _ -> !^"inl"))
                ^^ colon ^^ (name#p ^^ concat_map_with ~pre:space
                          (fun (x : generic_param) ->
                            match x with
                            | { ident; _ } ->
                                (self#_do_not_override_lazy_of_local_ident
                  ...
                  (!^"bind_both" ^^ space ^^ !^"x" ^^ space ^^ parens(!^"fun" ^^ space ^^ !^"x" ^^ space ^^ !^"=>" ^^ space ^^ !^"ret_both" ^^ space ^^
                 parens(List.fold_left
                  ~init:(if List.is_empty v#v.arguments then !^"tt" else !^"x")
                  ~f:(fun y x -> parens(x ^^ space ^^ y))
                  ((if Stdlib.(i != 0) then [ !^"inr" ] else []) @ List.init (List.length variants - 1 - i) ~f:(fun _ -> !^"inl"))
               ...
               ^^ CoqNotation.notation_name
                 ...
                 (parens
                    (
                      (if List.is_empty v#v.arguments then empty
                       else !^"fun" ^^ space ^^ !^"x" ^^ space ^^ !^"=>")
                      ^^
                      List.fold_left
                  ~init:(if List.is_empty v#v.arguments then !^"tt" else !^"x")
                  ~f:(fun y x -> parens(x ^^ space ^^ y))
               ...
               (* (parens(if List.is_empty v#v.arguments *)
               (*  then *)
               (*    (List.fold_left ~init:!^"tt" ~f:(fun y x -> parens(x ^^ space ^^ y)) ((if Stdlib.(i != 0) then [ !^"inr" ] else []) @ List.init (List.length variants - 1 - i) ~f:(fun _ -> !^"inl"))) *)
               (*  else *)
               (*    !^"fun" ^^ space ^^  ^^ space ^^ !^"=>" ^^ space ^^ (List.fold_left ~init:!^"x" ~f:(fun y x -> parens(x ^^ space ^^ y)) ((if Stdlib.(i != 0) then [ !^"inr" ] else []) @ List.init (List.length variants - 1 - i) ~f:(fun _ -> !^"inl"))) ^^ space ^^ colon ^^ space ^^ name#p *)
               (* )) *)
               ...
               (*           (List.fold_left ~init:!^"tt" ~f:(fun y x -> parens(x ^^ space ^^ y)) ((if Stdlib.(i != 0) then [ !^"inr" ] else []) @ List.init (List.length variants - 1 - i) ~f:(fun _ -> !^"inl"))) *)
               (*         else *)
               (*           !^"fun" ^^ space ^^ parens(string "x" ^^ space ^^ colon ^^ space ^^ separate_map (space ^^ !^"×" ^^ space) *)
               (*                                        (fun (ident, typ, attrs) -> *)
               (*                                           ( self#_do_not_override_lazy_of_ty *)
               (*                                               AstPos_variant__arguments typ )#p) *)
               (*                                        v#v.arguments) ^^ space ^^ !^"=>" ^^ space ^^ (List.fold_left ~init:!^"x" ~f:(fun y x -> parens(x ^^ space ^^ y)) ((if Stdlib.(i != 0) then [ !^"inr" ] else []) @ List.init (List.length variants - 1 - i) ~f:(fun _ -> !^"inl"))) ^^ space ^^ colon ^^ space ^^ name#p *)
               (*        )) *)
             )
             variants)
      ...
      method literal_Int ~value ~negative ~kind:_ =
        parens(!^"ret_both" ^^ space ^^ (if negative then !^"-" else empty) ^^ string value)
      ...
      method param ~pat ~typ ~typ_span:_ ~attrs:_ =
        parens (pat#p ^^ space ^^ colon ^^ space ^^ !^"both" ^^ space ^^ typ#p)
      ...
      method printer_name = "SSProve printer"
      ...
      method item'_Enum_Variant ~name ~arguments ~is_record ~attrs:_ =
        if is_record then
          ...
          ^^ separate_map
               (space ^^ string "->" ^^ space)
               (fun (ident, typ, attr) -> parens(!^"both" ^^ space ^^ parens(typ#p)))
               arguments
