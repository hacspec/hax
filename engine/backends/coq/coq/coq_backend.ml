  ...
  class printer =
    ...
    object (self)
      ...
      method arm' ~super:_ ~arm_pat ~body ~guard:_ =
        space ^^ arm_pat#p ^^ space ^^ string "=>" ^^ nest 2 (break 1 ^^ body#p)
      ...
      method expr'_Loop ~super:_ ~body ~kind ~state ~control_flow:_ ~label:_
          ~witness:_ =
        kind#p ^^ space
      ...
      method literal_Char x1 = string "\"" ^^ string (Char.escaped x1) ^^ string "\"" ^^ string "%char"
...
class type printer_type =
  object
    ...
    method _do_not_override_item'_Type :
      ...
      generics:(generics Generic_printer.LazyDoc.lazy_doc * generic_param Generic_printer.LazyDoc.lazy_doc list *
