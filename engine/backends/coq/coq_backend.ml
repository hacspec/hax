...
module CoqBackend = struct
    ...
    | C.AST.Array (t :: ts) ->
        ( "array_from_list" ^ " " ^ "_" ^ " " ^ "["
          ^ List.fold_left
              ...
              ~f:(fun x y ->
                x ^ ";" ^ term_to_string_without_paren y (depth + 1))
              ts
          ^ "]",
          true )
  ...
  and inductive_case_args_to_string variants pre mid post :
    ...
    | x :: xs ->
        ...
        in
        let variant_definitions, variants_str =
          inductive_case_args_to_string xs pre mid post
        in
  ...
  and variants_to_string variants pre post : C.AST.decl list * string =
    ...
    | (ty_name, ty) :: xs ->
        let ty_definitions, ty_str = ty_to_string ty in
        let variant_definitions, variants_str =
          variants_to_string xs pre post
        in
  ...
  struct
    ...
    let rec pty (t : ty) : C.AST.ty =
      ...
      | TArray { typ; length } -> C.AST.ArrayTy (pty typ, Int.to_string length)
    ...
    and args_ty (args : generic_value list) : C.AST.ty list =
      (* List.map ~f:pty *)
      match args with
      | arg :: xs ->
          (match arg with
    ...
    let rec pexpr (e : expr) : C.AST.term =
          ...
          C.AST.Match
            ( pexpr scrutinee,
              List.map
                ...
                arms )
      | Ascription { e; typ } -> __TODO_term__ "asciption"
      ...
      | Construct { constructor; fields; base } -> __TODO_term__ "construct"
      | Closure { params; body } ->
      ...
      | MacroInvokation
          {
            macro =
              `Concrete
                Non_empty_list.
                  { crate = "hacspec_lib_tc"; path = [ "secret_bytes" ] };
            args;
            witness;
          } ->
          let open Hacspeclib_macro_parser in
          let o : SecretBytes.t =
            SecretBytes.parse args |> Result.ok_or_failwith
          in
      ...
      | MacroInvokation
          {
            macro =
              `Concrete
                Non_empty_list.
                  { crate = "hacspec_lib_tc"; path = [ "secret_array" ] };
            args;
            witness;
          } ->
          let open Hacspeclib_macro_parser in
          let o : SecretArray.t =
            SecretArray.parse args |> Result.ok_or_failwith
          in
          (* let typ = *)
          (*   match o.typ with *)
          (*   | "U32" -> C.AST.U32 *)
    ...
    let rec pitem (e : item) : C.AST.decl list =
      ...
      | Fn { name; generics; body; params } ->
          [
            C.AST.Definition
              ( pglobal_ident name,
                List.map
                  ~f:(fun { pat; typ; typ_span } -> (ppat pat, pty typ))
                  params,
                pexpr body,
                pty body.typ );
          ]
      ...
      | Type
          ...
          [
            C.AST.Record
              (pglobal_ident_last name ^ "_t", p_record_record arguments);
          ]
      (* enum *)
      ...
      | Type { name; generics; variants } ->
          [
            C.AST.Notation
              ( pglobal_ident_last name ^ "_t",
                C.AST.Product (List.map ~f:snd (p_record variants name)) );
            C.AST.Definition
      ...
      | IMacroInvokation
          ...
          [
            C.AST.Notation
              ( o.array_name ^ "_t",
                C.AST.ArrayTy
                  (C.AST.Int (typ, false), (* int_of_string *) o.size) );
            C.AST.Definition
              ( o.array_name,
                [],
                C.AST.Var "id",
              ...
              ( pglobal_ident name,
                List.map
                  ~f:(fun x ->
                    ( x.ti_name,
                      match x.ti_v with
                      | TIFn fn_ty -> pty fn_ty
                      | _ -> __TODO_ty__ "field_ty" ))
                  items,
                List.fold_left ~init:[]
                  ~f:(fun a b ->
                    a
    ...
    and p_inductive variants parrent_name : C.AST.inductive_case list =
      ...
      | { name; arguments = [ (arg_name, arg_ty) ] } :: xs ->
          if (index_of_field >> Option.is_some) arg_name then
            C.AST.InductiveCase (pglobal_ident_last name, pty arg_ty)
            :: p_inductive xs parrent_name
          else
            C.AST.InductiveCase (pglobal_ident_last arg_name, pty arg_ty)
            :: p_inductive xs parrent_name
      | { name; arguments = [] } :: xs ->
      ...
      | { name; arguments } :: xs ->
          C.AST.InductiveCase
            ( pglobal_ident_last name,
              C.AST.RecordTy (pglobal_ident name, p_record_record arguments) )
          :: p_inductive xs parrent_name
      | _ -> []
    ...
    and p_record variants parrent_name : (string * C.AST.ty) list =
      ...
      | { name; arguments } :: xs ->
          ( pglobal_ident_last name,
            C.AST.RecordTy (pglobal_ident name, p_record_record arguments) )
          :: p_record xs parrent_name
      | _ -> []

    and p_record_record arguments : (string * C.AST.ty) list =
      match arguments with
      | (arg_name, arg_ty) :: xs ->
          (pglobal_ident_last arg_name, pty arg_ty) :: p_record_record xs
      | _ -> []
  end
  ...
  let hardcoded_coq_headers =
    ...
    \     Definition int_xI {WS : WORDSIZE} (a : int) : int := \
     MachineIntegers.add (MachineIntegers.mul a (repr 2)) MachineIntegers.one.\n\
     Definition int_xO {WS : WORDSIZE} (a : int) : int := MachineIntegers.mul \
     a (repr 2).\n\
     Number Notation int Pos.of_num_int Pos.to_num_int (via positive mapping \
     [[int_xI] => xI, [int_xO] => xO , [MachineIntegers.one] => xH]) : \
     hacspec_scope.\n\
     Notation \"0\" := (repr O).\n\
     Notation U8_t := int8.\n\
     Notation U8 := id.\n\
     ...
     Notation U128 := id.\n\n\
     Definition array_index {A: Type} `{Default A} {len : nat} (s: nseq A len) \
     {WS} (i : @int WS) := array_index s (unsigned i).\n\
     Notation \" x .[ a ]\" := (array_index x a) (at level 40).\n\
     Definition array_upd {A: Type} {len : nat} (s: nseq A len) {WS} (i: @int \
     WS) (new_v: A) : nseq A len := array_upd s (unsigned i) new_v.\n\
     Notation \" x .[ i ]<- a\" := (array_upd x i a) (at level 40).\n\n\
     Class Addition A := add : A -> A -> A.\n\
     Notation \"a .+ b\" := (add a b).\n\
     Instance array_add_inst {ws : WORDSIZE} {len: nat} : Addition (nseq (@int \
     ws) len) := { add a b := a array_add b }.\n\
     Instance int_add_inst {ws : WORDSIZE} : Addition (@int ws) := { add a b \
     := MachineIntegers.add a b }.\n\n\
     Class Subtraction A := sub : A -> A -> A.\n\
     Notation \"a .- b\" := (sub a b).\n\
     Instance array_sub_inst {ws : WORDSIZE} {len: nat} : Subtraction (nseq \
     (@int ws) len) := { sub := array_join_map MachineIntegers.sub }.\n\
     Instance int_sub_inst {ws : WORDSIZE} : Subtraction (@int ws) := { sub a \
     b := MachineIntegers.sub a b }.\n\n\
     Class Multiplication A := mul : A -> A -> A.\n\
     Notation \"a .* b\" := (mul a b).\n\
     Instance array_mul_inst {ws : WORDSIZE} {len: nat} : Multiplication (nseq \
     (@int ws) len) := { mul a b := a array_mul b }.\n\
     Instance int_mul_inst {ws : WORDSIZE} : Multiplication (@int ws) := { mul \
     a b := MachineIntegers.mul a b }.\n\n\
     Class Xor A := xor : A -> A -> A.\n\
     Notation \"a .^ b\" := (xor a b).\n\n\
     Instance array_xor_inst {ws : WORDSIZE} {len: nat} : Xor (nseq (@int ws) \
     len) := { xor a b := a array_xor b }.\n\
     Instance int_xor_inst {ws : WORDSIZE} : Xor (@int ws) := { xor a b := \
     MachineIntegers.xor a b }.\n\n\
     Definition new {A : Type} `{Default A} {len} : nseq A len := array_new_ \
     default _.\n\
     ...
     Instance seq_array_or_seq {A} `{Default A} (a : seq A) : array_or_seq A \
     (length a) :=\n\
     { as_seq := a ; as_nseq := array_from_seq _ a ; }.\n\
     Coercion seq_array_or_seq : seq >-> array_or_seq.\n\n\
     Definition update {A : Type}  `{Default A} {len slen} (s : nseq A len) \
     {WS} (start : @int WS) (start_a : array_or_seq A slen) : nseq A len :=\n\
     array_update (a := A) (len := len) s (unsigned start) (as_seq start_a).\n\n\
     Definition to_le_U32s {A l} := array_to_le_uint32s (A := A) (l := l).\n\
     Axiom to_le_bytes : forall {ws : WORDSIZE} {len}, nseq (@int ws) len -> \
     seq int8.\n\
     Definition from_seq {A : Type}  `{Default A} {len slen} (s : array_or_seq \
     A slen) : nseq A len := array_from_seq _ (as_seq s).\n\n\
     Notation Seq_t := seq.\n\
     Notation len := (fun s => seq_len s : int32).\n\n\
     Definition array_slice {a: Type} `{Default a} {len : nat} (input: nseq a \
     len) {WS} (start: @int WS) (slice_len: @int WS) : seq a := slice \
     (array_to_seq input) (unsigned start) (unsigned (start .+ slice_len)).\n\
     Notation slice := array_slice.\n\
     Definition seq_new {A: Type} `{Default A} {WS} (len: @int WS) : seq A := \
     seq_new (unsigned len).\n\
     Notation new_seq := seq_new.\n\
     Notation num_exact_chunks := seq_num_exact_chunks.\n\
     Notation get_exact_chunk := seq_get_exact_chunk.\n\
     Definition set_chunk {a: Type} `{Default a} {len} (s: seq a) {WS} \
     (chunk_len: @int WS) (chunk_num: @int WS) (chunk: array_or_seq a len) : \
     seq a := seq_set_chunk s (unsigned chunk_len) (unsigned chunk_num) \
     (as_seq chunk).\n\
     Definition set_exact_chunk {a} `{H : Default a} {len} s {WS} := \
     @set_chunk a H len s WS.\n\
    \     Notation get_remainder_chunk := seq_get_remainder_chunk.\n\
     ...
     Instance nat_mod_addition {n} : Addition (nat_mod n) := { add a b := a +% \
     b }.\n\
     Instance nat_mod_subtraction {n} : Subtraction (nat_mod n) := { sub a b \
     := a -% b }.\n\
     Instance nat_mod_multiplication {n} : Multiplication (nat_mod n) := { mul \
     a b := a *% b }.\n\
     Definition from_slice {a: Type} `{Default a} {len slen} (x : array_or_seq \
     a slen) {WS} (start: @int WS) (slice_len: @int WS) := array_from_slice \
     default len (as_seq x) (unsigned start) (unsigned slice_len).\n\
     ...
     Definition int_to_int {ws1 ws2} (i : @int ws1) : @int ws2 := repr \
     (unsigned i).\n\
     ...
     Instance nat_mod_znz_addition {n} : Addition (GZnZ.znz n) := { add a b := \
     a +% b }.\n\
     Instance nat_mod_znz_subtraction {n} : Subtraction (GZnZ.znz n) := { sub \
     a b := a -% b }.\n\
     Instance nat_mod_znz_multiplication {n} : Multiplication (GZnZ.znz n) := \
     { mul a b := a *% b }.\n\
     ...
     Fixpoint Build_Range_t (a b : nat) := match (b - a)%nat with O => [] | S \
     n => match b with | O => [] | S b' => Build_Range_t a b' ++ [b] end end.\n\
