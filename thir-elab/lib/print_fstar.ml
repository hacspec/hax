...
let rec pty (t : ty) =
  ...
  | TApp { ident = `TupleType n; args } when n >= 2 -> (
      let args =
        List.filter_map ~f:(function GType t -> Some (pty t) | _ -> None) args
      in
  ...
  | TProjectedAssociatedType s ->
      ...
      F.term
      @@ F.AST.Const (F.Const.Const_string ("TODO:ProjectedType", F.dummyRange))
  ...
  | PConstruct { name; args; record } ->
      ...
      if record then pat_rec ()
        ...
        let is_payload_record =
          List.for_all ~f:(fun { field } -> is_field_an_index field) args |> not
        in
        F.pat_app pat_name
        @@
...
let rec pexpr (e : expr) =
  ...
  | App
      ...
      F.term @@ F.AST.Project (pexpr arg, F.lid [ "_" ^ string_of_int (n + 1) ])
  ...
  | Match { scrutinee; arms } ->
      F.term @@ F.AST.Match (pexpr scrutinee, None, None, List.map ~f:parm arms)
  | Ascription { e; typ } ->
...
let rec pgeneric_param_bd (p : generic_param) =
  ...
  | GPType { ident; default = None } ->
      let t = F.term @@ F.AST.Name (F.lid [ "Type" ]) in
      F.mk_e_binder (F.AST.Annotated (plocal_ident ident, t))
  | GPType { ident; default = _ } -> failwith "pgeneric_param:Type with default"
