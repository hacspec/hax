...
let mk_functor_typ'
      loc
      (functor_body: module_type)
      (arg: string * module_type)
    : module_type
  = let x_name, x_type = arg in
    { pmty_desc =
        Pmty_functor
          ( Named ( { txt = Some x_name; loc }
                  , x_type )
          , functor_body
          )
    ; pmty_loc = loc
...
let nonrec_types_of_module_type (mtyp: module_type): type_declaration list
  = mtyp
    |> types_of_module_type
    |> List.concat_map ~f:(fun (_, l) -> l)
...
let mk_ptyp_arrow loc (types: core_type list): core_type =
  ...
  in List.fold_right ~init ~f:(fun a b -> wrap @@ Ptyp_arrow (Nolabel, a, b)) tl
...
let expand ~(ctxt : Expansion_context.Extension.t) (self_name : string)
  ...
  let feature_subtype_sig =
    mk_functorized_module_type ~loc ~name:feature_subtype_name ~body:{
        pmty_desc =
          Pmty_signature (
              nonrec_types_of_module_type features_mod_type
              |> List.map ~f:(fun typ ->
                     let h n = mk_ptyp_constr loc (mk_longident2 n typ.ptype_name.txt) in
                     let typ_decl = {
                         ...
                         ptype_manifest =
                           Some ( mk_core_type loc
                                  @@ Ptyp_arrow(Nolabel, h "A", h "B"));
                         ptype_attributes = [];
                         ptype_loc = typ.ptype_loc;
                       }
                     in { psig_desc = Psig_type(Recursive, [typ_decl])
  ...
  let self_str = match self.pmod_desc with
    | Pmod_structure s -> s
             ...
             Pstr_value
               ...
               , List.map ~f:(
                     ...
                     let mk =
                       ...
                       {
                         pvb_pat =
                           [%pat? ([%p name]: [%t mk_ptyp_arrow decl.ptype_loc typs])
                           ];
                         (* pvb_pat = { *)
                         (*   ppat_desc = Ppat_var decl.ptype_name; *)
                         (*   ppat_loc = decl.ptype_loc; *)
                     ...
                     | Ptype_abstract -> failwith "cannot deal with abstract type definitions"
                     | Ptype_open -> failwith "cannot deal with open"
                     | Ptype_record _ -> mk
                     | Ptype_variant cl -> mk
                   ) l)
           )
      |> List.map ~f:(fun pstr_desc -> { pstr_desc
                                       ; pstr_loc = loc })
      (* |> (fun s -> ) *)
    in
    mk_functorized_module ~loc ~name:"Subtype"
      ~body:{
        pmod_desc =
          Pmod_structure
            ([
                [%stri module MA = Hello(A)];
                [%stri module MB = Hello(B)]
              ] @ items)
