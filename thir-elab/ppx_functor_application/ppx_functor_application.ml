...
let rec elab ~loc (t: module_dsl): module_expr =
  ...
  | App ((Abs (arg, m) | Meta (Abs (arg, m), _)), x) ->
    E.pmod_structure [
      E.pstr_module @@ E.module_binding
        ~name:{loc; txt = Some arg}
        ~expr:(h x);
  ...
  | Pipe (x::funs) ->
    ...
    let binds
      = List.mapi ~f:(fun i _ -> E.pmod_ident {txt = Lident (nth_arg @@ i + 1); loc}) funs
...
let rec collect_pipes (t: module_dsl): module_dsl list
  = match t with
  | Meta (Pipe l, _)
  | Pipe l -> List.concat_map ~f:collect_pipes l
  | _ -> [t]
...
let rec normalize (t: module_dsl): module_dsl =
  ...
  | Pipe _ -> match collect_pipes t with
    | [] -> failwith "Empty pipe"
    | [t] -> t
    | l -> Pipe l
...
let rec parse expr =
  ...
  let r = match expr with
    ...
    | {pexp_desc = Pexp_construct ({txt}, Some arg)} -> App (Var txt, parse arg)
