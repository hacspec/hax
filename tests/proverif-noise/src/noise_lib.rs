...
pub fn encrypt_with_ad(cs:CipherState,ad:&Seq<U8>,plaintext:&Seq<U8>) -> Result<(CipherState,Seq<U8>),Error> {
    let CipherState {k,n} = cs;
    if n == 0xffffffffffffffffu64 {Err(Error::CryptoError)}
    else {
        match k {
            Some(k) => {
                let cip = encrypt(&k,n,ad,plaintext);
                Ok((CipherState {k:Some(k),n:n+1}, cip))
            },
...
pub fn decrypt_with_ad(cs:CipherState,ad:&Seq<U8>,ciphertext:&Seq<U8>) -> Result<(CipherState,Seq<U8>),Error> {
    let CipherState {k,n} = cs;
    if n == 0xffffffffffffffffu64 {Err(Error::CryptoError)}
    else {
        match k {
            Some(k) => {
                ...
                Ok((CipherState {k:Some(k),n:n+1}, plain))
            }
...
pub fn initialize_symmetric(protocol_name:&Seq<U8>) -> SymmetricState {
    ...
    let ck = hv.clone();
    SymmetricState {cs:initialize_key(None), ck, h : hv}
}
...
pub fn mix_key(st:SymmetricState,input_key_material:&Seq<U8>) -> SymmetricState {
    ...
    SymmetricState {cs:initialize_key(Some(temp_k)),ck,h}
}

pub fn mix_hash(st:SymmetricState,data:&Seq<U8>) -> SymmetricState {
    let SymmetricState {cs,ck,h} = st;
    SymmetricState {cs,ck,h:hash(&h.concat(data))}
}
...
pub fn mix_key_and_hash(st:SymmetricState,input_key_material:&Seq<U8>) -> SymmetricState {
    ...
    SymmetricState {cs:initialize_key(Some(temp_k)),ck,h:new_h}
}
...
pub fn encrypt_and_hash(st:SymmetricState,plaintext:&Seq<U8>) -> Result<(SymmetricState,Seq<U8>),Error> {
    let (new_cs,ciphertext) = encrypt_with_ad(st.cs,&st.h,plaintext)?;
    let new_h = hash(&st.h.concat(&ciphertext));
    Ok((SymmetricState {cs:new_cs,ck:st.ck,h:new_h},ciphertext))
}

pub fn decrypt_and_hash(st:SymmetricState,ciphertext:&Seq<U8>) -> Result<(SymmetricState,Seq<U8>),Error> {
    let (new_cs,plaintext) = decrypt_with_ad(st.cs,&st.h,ciphertext)?;
    let new_h = hash(&st.h.concat(ciphertext));
    Ok((SymmetricState {cs:new_cs,ck:st.ck,h:new_h},plaintext))
}
...
pub fn split(st:SymmetricState) -> (CipherState,CipherState,Seq<U8>) {
    ...
    (initialize_key(Some(temp_k1)),initialize_key(Some(temp_k2)),st.h)
}
