...
fn test_sha256_kat() {
    ...
    let digest = hash(&msg.classify_each());
    println!("{:?}", expected_256); 
    println!("{:x?}", digest.declassify_each());
    assert_eq!(expected_256, digest.declassify_each());
...
fn empty_input() {
    ...
    assert_eq!(hash(&vec![]).declassify_each(), SHA256_EMPTY);
}
