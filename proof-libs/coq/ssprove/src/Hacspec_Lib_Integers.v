...
Program Instance int_add_inst {ws : wsize} : t_Add (@int ws) (@int ws) := {
    f_Output := int ws ;
    f_add_loc := fset [] ;
    f_add {L1 L2 I1 I2} a b := int_add a b
  }.
Solve All Obligations with now intros ; rewrite <- fset0E, fsetU0.
Fail Next Obligation.

Program Instance int_sub_inst {ws : wsize} : t_Sub (@int ws) (@int ws) := {
    f_Output := int ws ;
    f_sub_loc := fset [] ;
    f_sub {L1 L2 I1 I2} a b := int_sub a b
  }.
Solve All Obligations with now intros ; rewrite <- fset0E, fsetU0.
Fail Next Obligation.

Program Instance int_mul_inst {ws : wsize} : t_Mul (@int ws) (@int ws) := {
    f_Output := int ws ;
    f_mul_loc := fset [] ;
    f_mul {L1 L2 I1 I2} a b := int_mul a b
  }.
Solve All Obligations with now intros ; rewrite <- fset0E, fsetU0.
Fail Next Obligation.

Program Instance int_xor_inst {ws : wsize} : t_Xor (@int ws) (@int ws) := {
    f_Output := int ws ;
    f_xor_loc := fset [] ;
    f_xor {L1 L2 I1 I2} a b := int_xor a b
  }.
Solve All Obligations with now intros ; rewrite <- fset0E, fsetU0.
Fail Next Obligation.
