...
Arguments Build_t_Unary.
Arguments Unary_0.
#[export] Instance settable_t_Unary : Settable _ :=
  settable! (Build_t_Unary) <Unary_0>.
Notation "'Unary'" := Build_t_Unary.

Inductive t_UNARY : Type :=
| UNARY_ZERO
| UNARY_SUCC : t_Unary -> _.
Arguments UNARY_ZERO.
Arguments UNARY_SUCC.

Definition unary_from_int (x : t_HaxInt) : t_Unary :=
  Build_t_Unary (x).

Definition unary_to_int (s : t_Unary) : t_HaxInt :=
  Unary_0 s.

Instance t_Clone_289793624 : t_Clone ((t_Unary)) :=
  {
    Clone_impl_f_clone := fun  (self : t_Unary)=>
...
Definition match_unary (s : t_Unary) : t_UNARY :=
  if
    is_zero (unary_to_int (Clone_f_clone (s)))
  then
    UNARY_ZERO
  else
    UNARY_SUCC (pred (s)).
