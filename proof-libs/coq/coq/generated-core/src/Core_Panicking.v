...
From Core Require Import Core (t_fmt).
Export Core (t_fmt).



From Core Require Import Core_Panic (t_Location).
Export Core_Panic (t_Location).

From Core Require Import Core_Panic (t_PanicInfo).
Export Core_Panic (t_PanicInfo).

Inductive t_AssertKind : Type :=
| AssertKind_Eq
| AssertKind_Ne
| AssertKind_Match.
Arguments AssertKind_Eq.
Arguments AssertKind_Ne.
Arguments AssertKind_Match.
...
Fixpoint panic_fmt (fmt : t_Arguments) : t_Never :=
  panic_fmt (fmt).

Definition panic (expr : string) : t_Never :=
  panic_fmt (impl_2__new_const ([expr])).

Definition panic_display `{v_T : Type} `{t_Sized (v_T)} `{t_Display (v_T)} (x : v_T) : t_Never :=
  panic_fmt (impl_2__new_v1 ([""%string]) ([impl_1__new_display (x)])).

Definition panic_explicit '(_ : unit) : t_Never :=
  panic_display ("explicit panic"%string).
