...
Instance t_Into_730689925 `{v_T : Type} `{v_U : Type} `{t_Sized (v_T)} `{t_Sized (v_U)} `{t_From (v_U) (v_T)} : t_Into ((v_T)) ((v_U)) :=
  {
    Into_impl_f_into := fun  (self : v_T)=>
      From_f_from (self);
  }.
