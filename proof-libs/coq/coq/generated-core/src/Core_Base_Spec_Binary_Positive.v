...
From Core Require Import Core_Base_Spec_Binary_Super_Haxint.
Export Core_Base_Spec_Binary_Super_Haxint.

Record t_Positive : Type :=
  {
    Positive_0 : t_HaxInt;
  }.
Arguments Build_t_Positive.
Arguments Positive_0.
#[export] Instance settable_t_Positive : Settable _ :=
  settable! (Build_t_Positive) <Positive_0>.
Notation "'Positive'" := Build_t_Positive.

Inductive t_POSITIVE : Type :=
| POSITIVE_XH
| POSITIVE_XO : t_Positive -> _
| POSITIVE_XI : t_Positive -> _.
Arguments POSITIVE_XH.
Arguments POSITIVE_XO.
Arguments POSITIVE_XI.

Definition positive_from_int (x : t_HaxInt) : t_Positive :=
  Build_t_Positive (x).

Definition positive_to_int (s : t_Positive) : t_HaxInt :=
  Positive_0 s.

Definition xH : t_Positive :=
  Build_t_Positive (v_HaxInt_ONE).

Instance t_Clone_16961168 : t_Clone ((t_Positive)) :=
  {
    Clone_impl_f_clone := fun  (self : t_Positive)=>
...
Definition match_positive (s : t_Positive) : t_POSITIVE :=
  if
    match_positive__is_xH (Clone_f_clone (s))
  then
    POSITIVE_XH
  else
    if
      match_positive__is_xO (Clone_f_clone (s))
    then
      POSITIVE_XO (positive_from_int (div2 (positive_to_int (s))))
    else
      POSITIVE_XI (positive_from_int (div2 (positive_to_int (s)))).
