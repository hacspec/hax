...
Record t_HaxInt : Type :=
  {
    HaxInt_f_v : t_Cow ((t_Slice t_u8));
  }.
Arguments Build_t_HaxInt.
Arguments HaxInt_f_v.
#[export] Instance settable_t_HaxInt : Settable _ :=
  settable! (Build_t_HaxInt) <HaxInt_f_v>.

Definition v_HaxInt_ONE : t_HaxInt :=
  Build_t_HaxInt (Cow_Borrowed (unsize ([1]))).

Definition v_HaxInt_TWO : t_HaxInt :=
  Build_t_HaxInt (Cow_Borrowed (unsize ([2]))).

Definition v_HaxInt_ZERO : t_HaxInt :=
  Build_t_HaxInt (Cow_Borrowed (unsize ([]))).

Instance t_Clone_622375820 : t_Clone ((t_HaxInt)) :=
  {
    Clone_impl_f_clone := fun  (self : t_HaxInt)=>
